    ...
    <div className="results">
      ...
      <div className="routes-container">
        {routes && routes.map(route => {
          const hasCoordinates = route.coordinates && route.coordinates.start && route.coordinates.end;
          ...
          return (
            <div key={route.id} className="route-card">
              ...
              {expandedRoute === route.id && hasCoordinates && route.coordinates.bbox && (
                <div className="map-container">
                  <p className="map-info">
                    📍 {route.coordinates.start.lat.toFixed(2)}°, {route.coordinates.start.lon.toFixed(2)}° → {route.coordinates.end.lat.toFixed(2)}°, {route.coordinates.end.lon.toFixed(2)}°
                  </p>
                  <iframe
                    width="100%"
                    height="400"
              ...
              {expandedRoute === route.id && !hasCoordinates && (
                <div className="map-container">
                  <p style={{ color: '#666', padding: '20px' }}>Map data loading...</p>
                </div>
              )}
            </div>
          );
